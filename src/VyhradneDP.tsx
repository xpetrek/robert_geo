...
import { vyhradneDp } from "./useData";

// center={[48.67, 19.7]}

const VyhradneDP = () => {
  console.log(vyhradneDp.features.length);
  ...
  return (
    ...
    >
      <>
        ...
        />
        {getPolygonCoordinates(vyhradneDp).map(
            ...
            return (
              ...
              >
                <Popup maxWidth={1000} maxHeight={500}>
                  <h3>{coordinates.attributes.loz_naz}</h3>

                  <div>{JSON.stringify(coordinates.attributes)}</div>
                  <a
                    target="_blank"
                    href={`https://apl.geology.sk/geofond/rastre/PDF/${coordinates.attributes.rozhodnutie_posledné}`}
                  >
                    {`https://apl.geology.sk/geofond/rastre/PDF/${coordinates.attributes.rozhodnutie_posledné}`}
                  </a>
                </Popup>
              </Polyline>
            );
